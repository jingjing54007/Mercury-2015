//*****************************************************************************
//
// motor - Software PWM drivers for the TB6612FNG driver
// 
...
typedef struct TBMotor {
    // Motor 'abstract' function
    void (*SetMotor)(struct TBMotor *mtr, float input);
...
// motor needs 2 pins
static tTBMotor TBMotorBuffer[PIN_COUNT / 2];

static int TBMotorCount = 0;


// This function sets a motor speed
static void SetTBMotor(tTBMotor *mtr, float input) { 
...
tTBMotor *_InitializeTBMotor(tPin a, tPin b, tBoolean brake, tBoolean invert) {
    // Grab the next motor
    tTBMotor *mtr = &TBMotorBuffer[TBMotorCount++];
