...
//"                     will not automatically reconnect to the network." },
    {"ping",          CMD_ping,
		...
		{"who",           CMD_who,
                  " : get the identity of the board and the server"},
		{"server",           CMD_server,
...
bool isValid(char* possibleNum)
{
		int len = strlen(possibleNum);
		if (len != 3){ return false; }
		if (possibleNum[0] < '0' || possibleNum[0] > '2'){ return false; }
		if (possibleNum[1] < '0' || possibleNum[1] > '9'){ return false; }
		if (possibleNum[2] < '0' || possibleNum[2] > '9'){ return false; }
		int numAsInt = atoi(possibleNum);
		if (numAsInt < 0 || numAsInt > 255){ return false; }
		return true;
}
...
int CMD_server(int argc, char **argv)
{
  ...
  unsigned char     len = sizeof(_NetCfgIpV4Args_t);
		...
		int ipAdd =atoi(argv[1]);
		IP_ADDR        = 0xc0a80064;   // 192=0xC0, 168=0xa8, 0x00=0, 0x65 = 101
...
//*****************************************************************************
//
// TODO: Ask the server a question from the client.
//
//*****************************************************************************
int CMD_ask(int argc, char **argv)
{
  ...
  sl_NetCfgGet(SL_IPV4_STA_P2P_CL_GET_INFO,&IsDHCP,&len,       // 19
	...
	messageLen -= 1; //take out the last space
	char* question = (char*)malloc(messageLen + 5); //allocate anough space for the message + the 4 preceding characters + the 0 sentinel
	sprintf(question, "%d", SL_IPV4_BYTE(ipV4.ipV4,0));      // analog data type                     26
  question[3] = '<';  
	int questionIndex = 4;
	for (int i = 1; i < argc; i++)
	{
		strcpy(&question[questionIndex], argv[i]);
		questionIndex += strlen(argv[i]);
		question[questionIndex] = ' ';
		questionIndex++;
	}
	question[questionIndex - 1] = 0;
	UARTprintf("Query String is \"%s\"\n", question);
...
//*****************************************************************************
//
// TODO: Get the server and client identities
//
//*****************************************************************************
int CMD_who(int argc, char **argv)
{
   ...
   // WlanConnect();
    ...
    //Print the IP
    UARTprintf("The client is at IP: %d.%d.%d.%d\n",  SL_IPV4_BYTE(ipV4.ipV4,3), SL_IPV4_BYTE(ipV4.ipV4,2), SL_IPV4_BYTE(ipV4.ipV4,1), SL_IPV4_BYTE(ipV4.ipV4,0));
    UARTprintf("The server is at IP: %d.%d.%d.%d\n",  SL_IPV4_BYTE(IP_ADDR,3), SL_IPV4_BYTE(IP_ADDR,2), SL_IPV4_BYTE(IP_ADDR,1), SL_IPV4_BYTE(IP_ADDR,0));
